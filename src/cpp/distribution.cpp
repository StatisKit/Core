...
namespace statiskit
{
    ...
    double CategoricalUnivariateMixtureDistribution::pdf(const int& position) const
    {
        ...
        for (Index index = 0, max_index = this->get_nb_states(); index < max_index; ++index) {
            p += this->pi[index] * this->observations[index]->pdf(position);
        }
        return p;        
    }
    ...
    std::set< std::string > CategoricalUnivariateMixtureDistribution::get_values() const
    {
        ...
        for(Index index = 0, max_index = this->get_nb_states(); index < max_index; ++index)
        { 
            std::set< std::string > state_values = this->observations[index]->get_values();
            values.insert(state_values.cbegin(), state_values.cend());
        }
        return values;
    }
    ...
    {
        int lv = this->observations[0]->quantile(p);
        int rv = lv;
        for (Index index = 1, max_index = this->get_nb_states(); index < max_index; ++index) {
            int current = this->observations[index]->quantile(p);
            if (current < lv) {
                lv = current;
            } else if (current > rv) {
                rv = current;
        ...
        double lp = this->cdf(lv);
        double rp = this->cdf(rv);
        do {
            int mv = (rv + lv)/2;
            double mp = this->cdf(mv);
            if (mp < p) {
                lv = mv;
                lp = mp;
            } else {
    ...
    {
        double lv = this->observations[0]->quantile(p);
        double rv = lv;
        for (Index index = 1, max_index = this->get_nb_states(); index < max_index; ++index) {
            double current = this->observations[0]->quantile(p);
            if (current < lv) {
                lv = current;
            } else if (current > rv) {
                rv = current;
    ...
    { 
        if (observation.get_nb_components() != this->get_nb_components()) {
    ...
    std::unique_ptr< MultivariateEvent > MixtureSingularDistribution::simulate(unsigned int sum) const
    {
        ...
        return this->observations[index]->simulate(sum);
    }
}
